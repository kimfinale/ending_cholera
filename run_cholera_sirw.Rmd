...
library( Rcpp )
sourceCpp( "models/cholera_sirw.cpp" )

tstop <- 600
nday <- 365
dt <- 365
times <- seq( 0, tstop*nday, dt )
params <- c( beta=0.5, chi_1=1, chi_2=1, chi_3=1, chi_4=1 )
...
res <- res[ , -1 ] # remove the time dimension
S <- rowSums( res[ , 1 : (num_age_grp) ] )
I <- rowSums( res[ , (1+num_age_grp) : (2*num_age_grp) ] )
R <- rowSums( res[ , (1+2*num_age_grp) : (3*num_age_grp) ] )
V <- rowSums( res[ , (1+3*num_age_grp) : (4*num_age_grp) ] )
B <- res[ , 6*num_age_grp+1 ]

N <- S + I + R + V

df <- data.frame( t=times,S=S, I=I, R=R, V=V, N=N, B=B )
...
# Integrate ODEs
res <- rk( y=init_val, times=times, func=cholera_sirw, parms=params, method="rk45dp7" )
S <- res[ , 2 : (nag+1) ]
I <- res[ , (2+nag) : (2*nag+1) ]
R <- res[ , (2+2*nag) : (3*nag+1) ]
V <- res[ , (2+3*nag) : (4*nag+1) ]
CI <- res[ ,(2+4*nag) : (5*nag+1) ]
CV <- res[ ,(2+5*nag) : (6*nag+1) ]
N <- S + I + R + V
...
library( adaptMCMC )
m <- MCMC( p=log_posterior, n=1e5, init=start, scale=c(0.01,0.2,0.2,0.2), adapt=TRUE, acc.rate=0.234, fun=cholera_sirw, data=inc_obs_Jakarta, tstop=100, pyo=1e5, maxsteps=500 )
# tic <- Sys.time()
