...
List cholera_sirw( double t, NumericVector y, NumericVector params ){
  ...
  for(int i = 0; i < num_age_grp; ++i){
    Rprintf("rate_vacc[%d] %f at time %f\n", i, rate_vacc_campaign[i], t );
  }
  ...
  for(int i = 0; i < num_age_grp; ++i){
     if( i == 0 ){
       dSdt[i] = + births - (foi*rel_susc[i] + mu[i] + rate_vacc_campaign[i]*eff_vacc[i] + ag[i])*S[i] + rate_wane_vacc*V[i] + rate_wane_nat*R[i] ;
       dIdt[i] = + foi*rel_susc[i]*S[i] - (mu[i] + ag[i] + gamma)*I[i];
       dRdt[i] = + (1 - case_fatality[i])*gamma*I[i] - (mu[i] + ag[i] + rate_wane_nat + rate_vacc_campaign[i]*eff_vacc[i])*R[i];
       dVdt[i] = + rate_vacc_campaign[i]*eff_vacc[i]*(S[i] + R[i]) - (mu[i] + ag[i] + rate_wane_vacc)*V[i];		
     } else {
       dSdt[i] = + ag[i-1]*S[i-1] - (foi*rel_susc[i] + mu[i] + rate_vacc_campaign[i]*eff_vacc[i] + ag[i])*S[i] + rate_wane_vacc*V[i] + rate_wane_nat*R[i];
       dIdt[i] = + ag[i-1]*I[i-1] + foi*rel_susc[i]*S[i] - (mu[i] + ag[i] + gamma)*I[i];
       dRdt[i] = + ag[i-1]*R[i-1] + (1 - case_fatality[i])*gamma*I[i] - (mu[i] + ag[i] + rate_wane_nat + rate_vacc_campaign[i]*eff_vacc[i])*R[i];
       dVdt[i] = + ag[i-1]*V[i-1] + rate_vacc_campaign[i]*eff_vacc[i]*(S[i] + R[i]) - (mu[i] + ag[i] + rate_wane_vacc)*V[i];
     }
