...
List cholera_sirw( double t, NumericVector y, NumericVector params ){
  ...
  // if a new variable is created via NumericVector rate_vacc_campaign_susc( num_age_grp )
  // its value is reset to zero at every iteration, which means it should be recalculated
  NumericVector rate_vacc_campaign_susc = Environment::global_env()["rate_vacc_campaign_susc"];;
  NumericVector rate_vacc_campaign_removed = Environment::global_env()["rate_vacc_campaign_removed"];;
  // NumericVector rate_vacc_campaign = Environment::global_env()["cov_vacc_campaign"];
  ...
  // if( t >= start_vacc_campaign && t <= (start_vacc_campaign + 1e-3) && vacc_campaign ){
  //   rate_vacc_campaign_susc = S * cov_vacc_campaign / dur_vacc_campaign;
  //   rate_vacc_campaign_removed = R * cov_vacc_campaign / dur_vacc_campaign;
  // }
  rate_vacc_campaign_susc = S * cov_vacc_campaign / dur_vacc_campaign;
  rate_vacc_campaign_removed = R * cov_vacc_campaign / dur_vacc_campaign;
  if( t < start_vacc_campaign || t >= stop_vacc_campaign || !vacc_campaign ){
    rate_vacc_campaign_susc = rate_vacc_campaign_susc * 0;
    rate_vacc_campaign_removed = rate_vacc_campaign_removed * 0;
  } 
  // Rprintf("rate_vacc_susc[1] %f at time %f\n", rate_vacc_campaign_susc[1], t );
  // Rprintf("S = %f and R = %f at time %f\n", sum(S), sum(R), t );
  // for(int i = 0; i < 2; ++i){
  //   Rprintf("rate_vacc_susc[%d] %f at time %f\n", i, rate_vacc_campaign_susc[i], t );
  //   Rprintf("rate_vacc_removed[%d] %f at time %f\n", i, rate_vacc_campaign_removed[i], t );
  // }
  ...
  for(int i = 0; i < num_age_grp; ++i){
     if( i == 0 ){
       dSdt[i] = + births - (foi*rel_susc[i] + mu[i]+ ag[i])*S[i] + rate_wane_vacc*V[i] + rate_wane_nat*R[i] - rate_vacc_campaign_susc[i]*eff_vacc[i] ;
       dIdt[i] = + foi*rel_susc[i]*S[i] - (mu[i] + ag[i] + gamma)*I[i];
       dRdt[i] = + (1 - case_fatality[i])*gamma*I[i] - (mu[i] + ag[i] + rate_wane_nat)*R[i] - rate_vacc_campaign_removed[i]*eff_vacc[i];
       dVdt[i] = + (rate_vacc_campaign_susc[i] + rate_vacc_campaign_removed[i])*eff_vacc[i] - (mu[i] + ag[i] + rate_wane_vacc)*V[i];		
     } else {
       dSdt[i] = + ag[i-1]*S[i-1] - (foi*rel_susc[i] + mu[i] + ag[i])*S[i] + rate_wane_vacc*V[i] + rate_wane_nat*R[i] - rate_vacc_campaign_susc[i]*eff_vacc[i];
       dIdt[i] = + ag[i-1]*I[i-1] + foi*rel_susc[i]*S[i] - (mu[i] + ag[i] + gamma)*I[i];
       dRdt[i] = + ag[i-1]*R[i-1] + (1 - case_fatality[i])*gamma*I[i] - (mu[i] + ag[i] + rate_wane_nat)*R[i] - rate_vacc_campaign_removed[i]*eff_vacc[i];
       dVdt[i] = + ag[i-1]*V[i-1] + (rate_vacc_campaign_susc[i] + rate_vacc_campaign_removed[i])*eff_vacc[i] - (mu[i] + ag[i] + rate_wane_vacc)*V[i];
     }
     dCIdt[i] = frac_report*frac_symptom*foi*rel_susc[i]*S[i]; 
     dCVdt[i] = rate_vacc_campaign_susc[i] + rate_vacc_campaign_removed[i];
   }
