...
//   for(int i = 0; i < num_age_grp; ++i){
//      if( i == 0 ){
//        dSdt[i] = + births - (foi*rel_susc[i] + mu[i] + eff_vacc[i]*rate_vacc_campaign[i] + ag[i])*S[i] + rate_wane_vacc*V[i] + rate_wane_nat*R[i] ;
//        dIdt[i] = + foi*rel_susc[i]*S[i] - (mu[i] + ag[i] + gamma)*I[i];
//        dRdt[i] = + (1 - case_fatality[i])*gamma*I[i] - (mu[i] + ag[i] + rate_wane_nat + eff_vacc[i]*rate_vacc_campaign[i])*R[i];
//        dVdt[i] = + eff_vacc[i]*rate_vacc_campaign[i]*(S[i] + R[i]) - (mu[i] + ag[i] + rate_wane_vacc)*V[i];
//      } else {
//        dSdt[i] = + ag[i-1]*S[i-1] - (foi*rel_susc[i] + mu[i] + eff_vacc[i]*rate_vacc_campaign[i] + ag[i])*S[i] + rate_wane_vacc*V[i] + rate_wane_nat*R[i];
//        dIdt[i] = + ag[i-1]*I[i-1] + foi*rel_susc[i]*S[i] - (mu[i] + ag[i] + gamma)*I[i];
//        dRdt[i] = + ag[i-1]*R[i-1] + (1 - case_fatality[i])*gamma*I[i] - (mu[i] + ag[i] + rate_wane_nat + eff_vacc[i]*rate_vacc_campaign[i])*R[i];
//        dVdt[i] = + ag[i-1]*V[i-1] + eff_vacc[i]*rate_vacc_campaign[i]*(S[i] + R[i]) - (mu[i] + ag[i] + rate_wane_vacc)*V[i];
//      }
//      dCIdt[i] = frac_report*frac_symptom*foi*rel_susc[i]*S[i];
//      dCVdt[i] = rate_vacc_campaign[i]*(S[i] + R[i]); // doesn't inlcude eff_vacc to account for the number of vaccine doses adminstered
//    }
//   dBdt = rate_excretion*I_symptom - rate_decay*B;
// 
//   //output
